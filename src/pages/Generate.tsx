...
import { AlgoEmptyState } from '../components/AlgoEmptyState';
...
export function Generate() {
  ...
  useEffect(() => {
    ...
    const sys = buildSystemPrompt(personaForPrompt, effectiveSettings, algoData, db.getTweets(activeProfileId || undefined));
    const user = buildUserMessage({
      topic: topic || '(konu girilmedi)',
      persona: personaForPrompt, settings: effectiveSettings,
      recentTweets: db.getTweets(activeProfileId || undefined),
  ...
  const handleGenerate = useCallback(async () => {
    ...
    const user = buildUserMessage({ topic, persona: personaForPrompt, settings: effectiveSettings, recentTweets: db.getTweets(activeProfileId || undefined), xUserTweets: userTweets, radarItems, impressionType, angle, mediaMode, length, goal, variations, viralTweets: promptTweets, externalTrends });
      try {
        ...
        {!loading && results.length === 0 && !thread && !error && (
          <div className="space-y-3">
            {viralTweets.length > 0 ? (
              ...
              <AlgoEmptyState algoData={algoData} />
            )}
          </div>
        )}
      </div>
